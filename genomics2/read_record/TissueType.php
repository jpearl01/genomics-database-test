...
<!--
    ...
    # Descripition: TissueType base table for genomics database<BR>
-->
...
if (!isset($_GET['record'])){
    $sql = "SELECT * FROM TissueType WHERE TissCode = 'ACD'";
  }
elseif (isset($_GET['record'])){
    $sql = "SELECT * FROM  TissueType WHERE TissCode = ".$_GET['record'];
}
...
<html>
  <div id="main">
    <head>
      <title>TissueType</title><?php if($numRows!=1){echo "<p class='warning'><font color='red'> Too many rows returned, incorrect TissCode?</font></p>";}?>
      <link rel="stylesheet" href="/css/genomics.css">
      </head>
      <spacer><h2>TissueType</h2>
      <hr></spacer>
...
<table>
	...
	<INPUT type="text" id="TissCode" value="<?php echo $row['TissCode'] ?>"><BR>
      </td>
    </tr>
    <tr> 
      <td colspan="2">
