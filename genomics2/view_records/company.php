<!DOCTYPE HTML>
<!--
   # Title: company_table<BR>
   # Author: J Earl modified by M Schultz<BR>
   # Date: December 2011<BR>
   # Description: company table for genomics database<BR>
-->
...
elseif (isset($_GET['col']) && !isset($_GET['sort'])){
    $sql = "SELECT * FROM Company ORDER BY ".$_GET['col'];
}
elseif (isset($_GET['col']) && isset($_GET['sort'])){
  $sql = "SELECT * FROM Company ORDER BY ".$_GET['col']." ".$_GET['sort'];
}
	...
	<?php
	  ...
	  while($row = $result->fetch_assoc()){
		...
		<th>
		     <ul class="colDropdown">
		        <li><a href="">'.htmlentities($item, ENT_COMPAT, 'UTF-8').'</a>
		            <ul>
		                <li><a href="company.php?col='.$item.'">Ascending</a></li>
		                <li><a href="company.php?col='.$item.'&sort=DESC">Descending</a></li>
	    ...
	    foreach($row as $item => $value){
	      ...
	      //continue;
	      //}
	      //Create links for Comp_num
	      if ($item == 'Comp_num'){
		$displayed_name = htmlentities($value, ENT_COMPAT, 'ISO-8859-15');
		echo '<td nowrap>'.createLink('../read_record/Company',$value,$displayed_name).'</td>';
	      }
	      else{
