<!DOCTYPE HTML>
<!--
   # Title: institution_table<BR>
   # Author: J Earl modified by M Schultz<BR>
   # Date: December 2011<BR>
   # Description: institution table for genomics database<BR>
-->
...
elseif (isset($_GET['col']) && !isset($_GET['sort'])){
    $sql = "SELECT * FROM Institution ORDER BY ".$_GET['col'];
}
elseif (isset($_GET['col']) && isset($_GET['sort'])){
  $sql = "SELECT * FROM Institution ORDER BY ".$_GET['col']." ".$_GET['sort'];
}
  ...
  <div id="records">
	...
	<?php
	  ...
	  while($row = $result->fetch_assoc()){
		...
		<th>
		    <ul class="colDropdown">
		        <li><a href="">'.htmlentities($item, ENT_COMPAT, 'UTF-8').'</a>
		            <ul>
		                <li><a href="institution.php?col='.$item.'">Ascending</a></li>
		                <li><a href="institution.php?col='.$item.'&sort=DESC">Descending</a></li>
		...
		echo '<td nowrap>'.createLink('../read_record/Institution',$value,$displayed_name).'</td>';
	      }
	      else{
